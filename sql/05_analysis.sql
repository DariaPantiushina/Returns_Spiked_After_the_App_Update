...
added_to_cart AS (
    SELECT
        ...
        region,
        channel,
        COUNT(DISTINCT user_id) AS users_added_to_cart
    FROM cart_users
    GROUP BY period, device, region, channel
),
dropoff AS (
    SELECT
        c.period,
...
SELECT
    ...
    a.channel,
    d.users_dropoff,
    a.users_added_to_cart,
    ROUND(
        100.0 * d.users_dropoff / NULLIF(a.users_added_to_cart, 0),
        2
...
ORDER BY a.period, a.device, a.region, a.channel;